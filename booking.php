...
<body>
	...
	<h2 style = "color:green;text-align:center"><?php
			if(isset($_GET['bstatus'])){
				echo "Your Cylinder has been successfully Booked";
			}
		?></h2>
	<h2 style = "color:green;text-align:center"><?php
			if(isset($_GET['bstatus'])){
				...
				echo $_GET['bstatus'];
				echo ".  Use this ID to check Your Booking Status";
			}
		?></h2>
		...
		<div class = "card text-center w-25 my-auto shadow">
			...
			<div class = "card-body">
				<form action = "book1.php" method = "post">
					...
					<div class = "form-group">
						<label for = "box3" >Phone No.</label>
						<label for = "box3" >(Registerd Mobile No. with above id)</label>
						<input type= "text" id = "box3" class = "form-control" name = "phone" required />
					</div>
				</div>
				<div class = "card-body">
					<input type = "submit" class = "btn btn-primary w-100" value = "Book Cylinder" name = "" />
				</div>
			</form>
		</div>
		</div>
