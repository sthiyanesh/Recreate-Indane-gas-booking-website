	...
	if($con->connect_error){
		...
		if($stmt_result -> num_rows > 0){
			...
			if($data['Cn_Name'] == $name && $data['DOB'] == $dob){
				$stmt = $con->prepare("update cn_detail set Cn_Name = ? where Cn_ID = ?");
				$stmt->bind_param("ss", $new,$id);
				$stmt->execute();
				$stmt = $con->prepare("update cn_contact set Cn_Name = ? where Cn_ID = ?");
				$stmt->bind_param("ss", $new,$id);
				$stmt->execute();
				header("location:editname.php?bstatus=True");
			}else{
